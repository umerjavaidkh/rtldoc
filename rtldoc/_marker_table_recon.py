"""Vendored from Marker (datalab-to/marker) v2.0.0, marker/processors/table_recon.py

Copyright 2024 Vikas Paruchuri / Datalab.
Licensed under the Apache License, Version 2.0 -- full text in
vendor/marker_logic/LICENSE. Unmodified apart from this header.

Used by rtldoc ONLY as a fallback where its own rule-based detection yields no
grid or a single column: see _projection_grid in pipeline.py. Measured on the
43 hand-verified tables, projection alone scores 0.356 against rtldoc's 0.625,
so it is a gap-filler, never a replacement.
"""
